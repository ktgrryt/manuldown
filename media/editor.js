...
(function () {
    ...
    function isCaretAdjacentToInlineCode(range) {
        ...
        const resolveInlineCodeCandidate = (node, direction) => {
            ...
            if (current &&
                ...
                cursorManager) {
                if (direction === 'next' &&
                    typeof cursorManager._getLeadingInlineCodeElement === 'function') {
                    return cursorManager._getLeadingInlineCodeElement(current);
                }
                if (direction === 'prev' &&
                    typeof cursorManager._getTrailingInlineCodeElement === 'function') {
                    return cursorManager._getTrailingInlineCodeElement(current);
                }
            }
            return null;
        };
    ...
    function shouldUseNativeArrowForTopLine(e) {
        ...
        if ((e.key === 'ArrowLeft' || e.key === 'ArrowRight') &&
            cursorManager &&
            typeof cursorManager.hasActiveInlineCodeBoundaryState === 'function' &&
            cursorManager.hasActiveInlineCodeBoundaryState()) {
            return false;
        }
        if ((e.key === 'ArrowLeft' || e.key === 'ArrowRight') &&
    ...
    function handleArrowKeydown(e) {
        ...
        const isPlainInlineBoundaryArrow =
            (e.key === 'ArrowLeft' || e.key === 'ArrowRight') &&
            !e.shiftKey && !e.metaKey && !e.ctrlKey && !e.altKey &&
            cursorManager &&
            typeof cursorManager.hasActiveInlineCodeBoundaryState === 'function' &&
            cursorManager.hasActiveInlineCodeBoundaryState();
        if (isPlainInlineBoundaryArrow) {
            const moved = e.key === 'ArrowRight'
                ? cursorManager.moveCursorForward(notifyChange)
                : cursorManager.moveCursorBackward(notifyChange);
            if (moved) {
                e.preventDefault();
                e.stopPropagation();
    ...
    function handleKeydown(e) {
        ...
        const isActiveComposition = !!(
            e.isComposing ||
            isComposing ||
            compositionUpdateGate.composing
        );
        const shouldRoutePostCompositionArrow = shouldRouteHorizontalArrowAfterComposition(
            e,
            isActiveComposition
        );
        ...
        // Some WebView/IME combinations report keyCode 229 for a normal arrow
        // immediately after composition. Once composition itself has ended, the
        // inline boundary state must still receive that arrow instead of falling
        // back to native movement over invisible caret anchors.
        const isMacHorizontalCtrlNav = isMac &&
            e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey &&
            ((e.key || '').toLowerCase() === 'b' || (e.key || '').toLowerCase() === 'f');
        if (!isPlainHorizontalArrow && !isMacHorizontalCtrlNav &&
            cursorManager && typeof cursorManager.clearInlineCodeBoundaryState === 'function') {
            cursorManager.clearInlineCodeBoundaryState();
        }
